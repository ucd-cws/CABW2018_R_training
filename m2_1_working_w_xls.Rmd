...
(ref:tidydataHW) *Figure from Hadley Wickham's R For Data Science (see: [Chap 12](https://r4ds.had.co.nz/tidy-data.html#spreading-and-gathering))*

```{r tidydata, echo=FALSE, fig.cap="(ref:tidydataHW)"}
...
The good thing is these all involve the same basic code, with just a few minor tweaks.
...
`.RDS` or `.rds` files are similar in type to `.rda`. They are R specific, they are compressed, *but* they are limited to a single object, *and* you can name this object whatever you choose when you re-import/read it back into R.

...
knitr::include_graphics(here::here("figure", "data-science-wrangle.png"))
```
...
knitr::include_graphics(here::here("figure", "filter.jpg"))
```
...
```{r out.width = "100%", echo=FALSE, fig.cap = '*Illustration by @allison_horst.*'}
knitr::include_graphics(here::here("figure", "mutate.png"))
```
...
**Challenge 2**
 ...
 - Select the unique sample ID column (`SampleID_old`), latitude (`New_Lat`), longitude (`New_Long`), and `CSCI` columns. Reassign these columns to a dataset named `cscidat`, effectively overwriting the existing dataset.
...
knitr::include_graphics(here::here("figure", "relocate.png"))
```

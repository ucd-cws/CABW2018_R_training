...
(1) Before we move on, take a moment to create some additional variables named `flow` and `pool` in your script. 
...
We run these installations in the *Console*, because they only need to happen once. Now, let's add the following to our script file and run each line to attach them for use today:

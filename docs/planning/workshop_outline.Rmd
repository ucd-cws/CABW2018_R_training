...
## Module 2: Make a Map from A Spreadsheet
  ...
  - (10 min): Read data from excel spreadsheet
    - Talk about tidy data and get people to think about this (spreadsheet is a list of sites or gages with location information)
      - read in `Tab1` which is messy and breaks things
      - then read in `Tab2` which is "tidied" and doesn't break
  - (10 min): **Join** Data
    - Join these sites with the data downloaded in Module 1 (may need to run function first)
  - (10 min): Quick overview of **sf**
    - explain the 'sf' package and show website and "potential"
    - make data spatial (X/Y) with `{sf}`
  - (5 min): Make a map with `{mapview}`
  - (10 min): Introduce ggplot2:
    - gently introduce structure of ggplot
    - use data to show `geom_sf` and make a simple map with ggplot
    - reference mapping website, Robin Lovelace's site

## Module 3

- read in at least 2 shapefiles and/or .gpx files with sf
- look at data and do some data frame operation (mutate/join)
- do some spatial operation (buffer, intersection, nearest?)
- make a map with ggplot (show ggspatial?)
